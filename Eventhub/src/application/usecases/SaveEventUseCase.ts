...
class SaveEventUseCase {
    ...
    async execute(userId: string, payload: CreateEventInputs) {
        ...
        if (!payload.category) throw new Error("La catégorie est requis");

        try {
            const event = await this.eventRepository.save(userId, payload);
...
export default SaveEventUseCase;